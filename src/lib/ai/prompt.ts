...
書き起こしは「話者名: 発言」の形式で渡されます（話者名が「不明」の場合もあります）。
話者情報を最大限に活用してください。
...
- topics の points では、誰がどの立場・意見かが分かるよう、必要に応じて話者名を含める。
- todos の owner には、その作業を担当する話者名を入れる（発言の文脈から担当者が読み取れる場合は積極的に付与する）。読み取れない場合のみ null。
- due も読み取れない場合は null にする。
