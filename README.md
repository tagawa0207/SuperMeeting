...
- 🔎 **調査・社内情報検索** — 論点・未解決の問いの「🔎」や自由入力から調査。
  ...
  社内情報検索は差し替え可能なプロバイダ設計。Slack（`SLACK_USER_TOKEN`）と
  Confluence（`CONFLUENCE_BASE_URL` 等）のコネクタを実装済み（未設定時はモック）。Drive/Jira 等も後付け可能。
...
## 社内情報検索（Confluence 連携）

Atlassian の API トークンを設定すると、社内情報検索に Confluence（Cloud）の
ページ・ブログ記事の検索（CQL `siteSearch`）が加わります。Slack と併用でき、
両方の結果が 1 枚の社内カードに束ねられます。

1. [Atlassian アカウント設定](https://id.atlassian.com/manage-profile/security/api-tokens) で「API トークンを作成」
2. `.env.local` に以下の 3 つを設定して dev サーバを再起動

```bash
CONFLUENCE_BASE_URL=https://your-company.atlassian.net/wiki
CONFLUENCE_EMAIL=you@example.com        # Atlassian アカウントのメールアドレス
CONFLUENCE_API_TOKEN=...                # 1. で作成したトークン
```

検索結果は**一般（global）スペース限定**です。個人トークンの検索には自分の個人スペース
（下書き・私的メモ）が混ざるため、画面共有中のボードに表示されないよう CQL で除外しています
（`CONFLUENCE_INCLUDE_PERSONAL=1` で解除できますが、画面共有前提のため非推奨）。
...
src/                         （Web アプリ）
  ...
  lib/
    ...
    research/
      ...
      confluence.ts          Confluence コネクタ（CQL siteSearch、一般スペース限定）
...
- 社内情報検索の追加コネクタ（Drive / Jira / Box。Slack / Confluence は実装済み）
